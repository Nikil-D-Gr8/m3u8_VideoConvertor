...
# Try to import imageio_ffmpeg, but handle the case where it's not available
try:
...
class HLSConverter(QWidget):
    ...
    def get_ffmpeg_path(self):
        """Get FFmpeg executable path, handling both development and bundled scenarios."""
        
        # Check if we're in a PyInstaller bundle
        if getattr(sys, 'frozen', False):
            # We're in a bundled executable
            if hasattr(sys, '_MEIPASS'):
                # Try to find ffmpeg in the bundle
                ...
                for name in ffmpeg_names:
                    ffmpeg_path = os.path.join(bundle_dir, name)
                    if os.path.exists(ffmpeg_path):
                        return ffmpeg_path
                
                # If not found in bundle, try system PATH
                ...
                QMessageBox.critical(self, "FFmpeg Not Found", 
                    "FFmpeg executable not found. Please ensure FFmpeg is installed on your system.")
                return None
        
        # Development mode or not bundled
        if IMAGEIO_AVAILABLE:
            try:
                return get_ffmpeg_exe()
            except Exception as e:
                print(f"Failed to get FFmpeg from imageio_ffmpeg: {e}")
        ...
        # FFmpeg not found
        QMessageBox.critical(self, "FFmpeg Not Found", 
            "FFmpeg not found. Please install FFmpeg or ensure it's in your system PATH.")
        return None
    ...
    def start_conversion(self, input_files, output_dir, single_file):
        ...
        if not ffmpeg_path:
            return  # Error already shown in get_ffmpeg_path
